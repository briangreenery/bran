...
function gitFetch() {
  var command = util.format('git --git-dir="%s" fetch http://%s/repo.git %s:%s',
                            path.resolve(this.gitDir),
                            this.master,
                            this.branch,
                            this.branch);
...
function maybeGitClean() {
  ...
  var command =
    util.format('git --git-dir="%s" clean -qfd', path.resolve(this.gitDir));
...
function gitCheckout() {
  var command = util.format(
    'git --git-dir="%s" checkout -qf %s', path.resolve(this.gitDir), this.ref);

  return this.runCommand(command);
}

function readConfig() {
  return Q.nfcall(fs.readFile, path.join(this.workTree, '.bran.yml'));
}
...
function runCommand(command) {
  ...
  this.command = Command(command, this.tmpDir, this.workTree);
